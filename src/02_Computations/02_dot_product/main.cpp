...
int main() {
    set_benchmark_suite("02_Computations/02_dot_product");
    ...
    // Run benchmarks
    benchmark_comparison("Dot Product (1024 vectors)", scalar_benchmark, simd_soa_benchmark, 50);
    std::cout << std::endl;
    ...
    benchmark_comparison("AoS vs SoA", simd_aos_benchmark, simd_soa_benchmark, 50);
    std::cout << std::endl;
	...
	// For tiny workloads the SIMD setup costs outweigh the computation, so a lower
	// speedup (or even a slowdown) is expected and useful to point out to learners.
	benchmark_comparison("Single Dot Product (1000 iterations)", scalar_single_benchmark, simd_single_benchmark, 10);
